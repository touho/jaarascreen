		...
		programSheet.getCells({
			...
			for (let i = 0; i < cells.length - 3; i += PROGRAM_SHEET_COLUMNS) {
				...
				if (year > 1000 && year < 10000) {
					...
					// move all program by days:
					//entry.timestamp = new Date(entry.timestamp.getTime() - 1000 * 60 * 60 * 24 * 0);
					// console.log('CURRENT_DAY', CURRENT_DAY)
					if (CURRENT_DAY) {
						if (!program[entry.place]) {
							console.log('entry', entry)
							console.log('Changing day from', entry.timestamp)
						}
						entry.timestamp = new Date(entry.timestamp.getTime() + (Date.now() - CURRENT_DAY.getTime()))
						if (!program[entry.place]) {
							console.log('To', entry.timestamp)
						}
					}

					entry.endTime = new Date(entry.timestamp.getTime() + 1000 * 60 * entry.durationMinutes)

					if (entry.endTime < new Date(Date.now() - 1000 * 60 * 60 * 1))
						continue
